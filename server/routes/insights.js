...
router.get('/weekly', async (req, res) => {
  try {
    ...
    if (req.query.start) {
      ...
      report = await computeWeeklyReport({ userId: req.userId, startDate });
    } else {
      report = await generateCurrentWeeklyInsights({ userId: req.userId });
    }
...
router.get('/weekly/debug', async (req, res) => {
  try {
    ...
    const latestEntries = await Entry.find({ userId: req.userId })
      .sort({ date: -1 })
      .limit(3)
...
router.get('/range', async (req, res) => {
  try {
    ...
    const insights = await getInsightsForRange({ userId: req.userId, startDate: new Date(start), endDate: new Date(end) });
    res.json(insights);
  } catch (error) {
