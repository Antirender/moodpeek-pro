...
router.post('/', async (req, res) => {
  try {
    const { date, mood, city, tags, note, weather, activities } = req.body;
    if (!date || !mood) return res.status(400).json({ error: 'date and mood are required' });

    const entryDate = new Date(date);
    const existing = await Entry.findOne({ userId: req.userId, date: entryDate });
    ...
    const entryData = {
      ...
      activities: sanitizeTags(activities),
      note,
    };
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (activities) update.activities = sanitizeTags(activities);
    if (note !== undefined) update.note = note;
    if (weather !== undefined) update.weather = weather;

    // If updating date, ensure uniqueness
    if (update.date) {
      const conflict = await Entry.findOne({ userId: req.userId, date: update.date, _id: { $ne: id } });
      if (conflict) return res.status(409).json({ error: 'Another entry for this date already exists' });
    }

    const updated = await Entry.findOneAndUpdate({ _id: id, userId: req.userId }, update, { new: true });
