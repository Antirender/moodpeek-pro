...
async function getSeedUser() {
  const email = process.env.SEED_USER_EMAIL || 'demo@moodpeek.test';
  let user = await User.findOne({ email });
  if (!user) {
    user = new User({ email });
    await user.setPassword(process.env.SEED_USER_PASSWORD || 'demoPass123!');
    await user.save();
  }
  return user;
}
...
async function seedEntries(){
  ...
  await Entry.deleteMany({ note: 'seed', userId: user._id });
