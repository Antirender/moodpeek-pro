 ...
 */
async function computeWeeklyReport({ userId, startDate }) {
  if (!userId) {
    throw new Error('userId is required for weekly reports');
  }
  ...
  if (!currentWeekEntries.length) {
    ...
    const latestEntries = await Entry.find({ userId })
      .sort({ date: -1 })
      .limit(3)
  ...
  try {
    const rangeKey = `${start.toISOString().split('T')[0]}_weekly`;
    const sortedMoods = Object.entries(moodDistribution).sort((a, b) => b[1] - a[1]);
    const topMood = sortedMoods.length ? sortedMoods[0][0] : null;
    await Insight.findOneAndUpdate(
      { userId, range: rangeKey },
      {
        userId,
        range: rangeKey,
        topMood,
 ...
 */
async function generateCurrentWeeklyInsights({ userId }) {
  ...
  return computeWeeklyReport({ userId, startDate: startOfWeek });
}
 ...
 */
async function getInsightsForRange({ userId, startDate, endDate }) {
  try {
    // Validate dates
    const start = new Date(startDate);
